...
def add_student(request):
    ...
    if request.method == 'POST':
        if student_form.is_valid():
            ...
            mark_10 = student_form.cleaned_data.get('mark_10')
            mark_12 = student_form.cleaned_data.get('mark_12')
            ...
            try:
                ...
                user.mother_occupation = mother_occupation
                user.mark_10 = mark_10
                user.mark_12 = mark_12
                user.save()
                messages.success(request, "Successfully Added")
...
def manage_student(request):
    students = StudentData.objects.filter(user_type=3)
    context = {
        'students': students,
...
def edit_student(request, student_id):
    ...
    try:
        user = CustomUser.objects.create_user(
                    email=student.email, password=student.password, user_type=3, first_name=student.first_name, last_name=student.last_name)
        user.gender = student.gender
        user.address = student.address
        user.father_name = student.father_name
        user.father_occupation = student.father_occupation
        user.mother_name = student.mother_name
